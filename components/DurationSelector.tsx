...
const DurationSelector: React.FC<DurationSelectorProps> = ({ durations, selectedDuration, onSelectDuration }) => {
  return (
    <div className="relative w-full max-w-md mx-auto">
      <select
        value={selectedDuration}
        onChange={(e) => onSelectDuration(e.target.value as Duration)}
      ...
      >
        {durations.map((duration) => (
          <option key={duration} value={duration} className="bg-white text-slate-800">
            {duration}
          </option>
        ))}
      </select>
